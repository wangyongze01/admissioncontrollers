...
Follow [Use CFSSL to generate certificates](tlssl_generate_self_signed.md)
...
```
docker build . -t newharbor.ipinyou.com/alphadata/test-webhook:v1
docker push newharbor.ipinyou.com/alphadata/test-webhook:v1
```

```

# apply generated secret
kubectl --kubeconfig=/home/cdp/k8s/sy-aliyun4/config -nlearning apply -f ./tls/test-webhook-tls.yaml


kubectl --kubeconfig=/home/cdp/k8s/sy-aliyun4/config -nlearning apply -f rbac.yaml
kubectl --kubeconfig=/home/cdp/k8s/sy-aliyun4/config -nlearning apply -f deployment.yaml
kubectl --kubeconfig=/home/cdp/k8s/sy-aliyun4/config -nlearning get pods

# ensure above pods are running first

kubectl --kubeconfig=/home/cdp/k8s/sy-aliyun4/config -nlearning apply -f webhook.yaml
